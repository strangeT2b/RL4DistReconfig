...
# Evaluate Llama-3.1-8B-Instruct with vLLM author-format + system loss.
...
BASE_MODEL="${BASE_MODEL:-runs/llama31_8b_instruct/merged/sft_ce_ep3_1}"
ADAPTER_PATH="${ADAPTER_PATH:-}"
ADAPTER_NAME="${ADAPTER_NAME:-llama31_8b_instruct_sft_ce_merged}"
OUTPUT_DIR="${OUTPUT_DIR:-outputs/llama31_8b_instruct_sft_ce_merged__on__train_33_69_84_nodes_test}"
SPLIT="${SPLIT:-test}"
PROMPT_FORMAT="${PROMPT_FORMAT:-llama3_chat}"
NUM_SAMPLES="${NUM_SAMPLES:-1000}"
SAMPLE_MODE="${SAMPLE_MODE:-sequential}"
...
if [[ -n "${SAMPLE_SEED}" ]]; then
  SAMPLE_SEED_ARGS=(--sample_seed "${SAMPLE_SEED}")
fi
