...
# SFT with custom graph-penalty loss — alternative to CE-based SFT.
...
RUN_NAME="${RUN_NAME:-sft_full_xml_custom_ep3__on__train_33_69_84_nodes}"
...
CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-0,1} torchrun \
  ...
  --max_new_tokens 1200 \
  --model_name_hf local-repro-llama31-base-fullxml-custom \
  --tokenizer_name_hf local-repro-llama31-base-fullxml-custom-tokenizer \
  --custom_loss 1 \
